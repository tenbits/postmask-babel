...
var _mask = null;
module.exports = {
	initialize (optimizer, settings, mask, io) {
		_mask = mask;
		["function", "slot", "pipe", "event"].forEach(function (method) {			
			optimizer.registerOptimizer(method, optimizeNode);
		});
	}
};

function optimizeNode(node, ctx, next) {
